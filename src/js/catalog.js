...
function proceedToCheckout() {
    ...
    const cartData = {
        items: [
            {
                id: item.id,
                name: item.nama_makanan || item.name || '',
                // price = the displayed price on catalog (final price before any platform discounts)
                price: item.harga || 0,
    ...
    // Only set this automatically when running on localhost and no test user is present.
    try {
        ...
        console.warn('Could not set test user in localStorage:', err);
    }
